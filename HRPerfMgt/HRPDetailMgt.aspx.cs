...
public partial class HRPerMgt_HRPDetailMgt : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Request.QueryString["status"] == "PerMain")
                {
                    Title = "绩效基础数据维护";
                }
            }
    ...
    protected void BtnSubmit_Click(object sender, EventArgs e)
    {
        if(Label28.Text=="新增")
        {
            if (TxtNewType.Text == ""||TxtAperson.Text==""||TxtCperson.Text=="")
            {
            ...
            hRPtypeInfo.HRPAT_ID = Guid.NewGuid();
                ...
                hRPtypeInfo.HRPAT_M_State = DropDownList1.Text.Trim();
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        Panel4.Visible = false;
                        UpdatePanel4.Update();
                        ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "alert", "alert('新增考核类型成功，请选择考核项目及员工！')", true);
                    }
        ...
        if (Label28.Text == "编辑")
        {
            ...
            hRPtypeInfo.HRPAT_ID = new Guid(Label27.Text);
                ...
                DropDownList1.Text = Label44.Text;
                hRPtypeInfo.HRPAT_M_State = DropDownList1.Text;
    ...
    protected void Grid_Type_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Edit3")
        {
            ...
            Label44.Text = hRType.HRPAT_M_State;
            DataSet dtAperson = neiaL.Search_ForTeacher_HRDDetail("and UMUI_UserID = '" + hRType.HRPAT_APerson.Trim() + "' ");
            DataSet dtCperson = neiaL.Search_ForTeacher_HRDDetail("and UMUI_UserID = '" + hRType.HRPAT_CPerson.Trim() + "' ");
            ...
            DataSet dtApersonID = neiaL.Search_ForTeacher_HRDDetail("and UMUI_UserName = '" + hRType.HRPAT_APerson + "' ");
            DataSet dtCpersonID = neiaL.Search_ForTeacher_HRDDetail("and UMUI_UserName = '" + hRType.HRPAT_CPerson + "' ");
            TxtNewType.Text = hRType.HRPAT_PType;
            Label31.Text = hRType.HRPAT_APerson;
            Label32.Text = hRType.HRPAT_CPerson;
            DropDownList1.Text = Label44.Text.Trim();
            UpdatePanel_NewType.Update();
            Label29.Text = "";
            Label30.Text = "";
        }
    ...
    private int AlertSumEdit()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            if (new Guid(Convert.ToString(sumData.Tables[0].Rows[i]["HRPI_ItemID"])) != new Guid(Label49.Text))
            {
                sumEdit += Convert.ToInt32(sumData.Tables[0].Rows[i]["HRPI_StanScore"]);
            }
        }
        //TextBox1.Text = Convert.ToString(sumEdit);
        return sumEdit;
    }//编辑时当标准得分之和不为100分时发出警告
    ...
    protected void BtnSubmit_Item(object sender, EventArgs e)
    {
        if (LblState.Text == "新增")
        {
            if (TextBox_newItem1.Text == ""||TextBox_newItem3.Text==""
                ||TextBox_newItem4.Text==""||TextBox_newItem2.Text=="")
            {
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('标记*的为必填项，请填写完整！')", true);
                return;
            }
            ...
            try
            {
                int scoreFinal = Convert.ToInt32(TextBox1.Text) + Convert.ToInt32(hRPItemInfo.HRPI_StanScore);
                if (scoreFinal > 100)
                {
                    ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "alert", "alert('标准得分总和不能大于100分！')", true);
                    return;
                }
        ...
        if (LblState.Text == "编辑")
        {
                ...
                try
                {
                    int scoreFinal = AlertSumEdit();
                    scoreFinal += Convert.ToInt32(hr.HRPI_StanScore);
                    if (scoreFinal > 100)
                    {
                        ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "alert", "alert('标准得分总和不能大于100分！')", true);
                        return;
    ...
    protected void BtnAddReset_Click(object sender, EventArgs e)
    {
        ...
        BindGridForEmployee(GridViewAdd, " and HRPAT_ID is null and SAS_ASID is not null");
    }//员工新增栏的“重置”
    ...
    protected void GridViewAdd_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        ...
        {
            BindGridForEmployee(GridViewAdd, con2 + " and HRPAT_ID is null and SAS_ASID is not null");
        }
        newPageIndex = newPageIndex < 0 ? 0 : newPageIndex;
    ...
    protected void Grid_Type_RowDataBound_Item(object sender, EventArgs e)
    {
