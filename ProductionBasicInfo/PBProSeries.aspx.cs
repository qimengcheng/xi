...
using System.Data;
public partial class ProductionBasicInfo_PBProSeries : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (state == "look")
            {
                ...
                GridView_ProType.Columns[0].Visible = false;
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (!((Session["UserRole"].ToString().Contains("产品系列管理")) || (Session["UserRole"].ToString().Contains("产品系列查看"))))
                    {
