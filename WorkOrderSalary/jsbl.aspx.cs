...
public partial class WorkOrderSalary_jsbl : System.Web.UI.Page
{
    ...
    {
        string condition;
        string JJBL_NO = TextBox_xiangmu.Text.Trim() == "" ? " and 1=1 " : " and JSBL_NO like '%" + TextBox_xiangmu.Text.Trim() + "%' ";
        string JJBL_Man = TextBox_tibaoren.Text.Trim() == "" ? " and 1=1 " : " and JSBL_Man like '%" + TextBox_tibaoren.Text.Trim() + "%' ";
        string JJBL_ShMan = TextBox_chushengren.Text.Trim() == "" ? " and 1=1 " : " and JSBL_ShMan like '%" + TextBox_chushengren.Text.Trim() + "%' ";
        string JJBL_State = DropDownList1.SelectedItem.Text.Trim() == "请选择" ? " and 1=1 " : " and JSBL_State like '%" + DropDownList1.SelectedItem.Text.Trim() + "%' ";
        ...
        string JJBL_Time = (TextBox_WO_Time1.Text.Trim() == "" && TextBox_WO_Time2.Text.Trim() == "") ? " and 1=1 " : " and JSBL_Time between   '" + TextBox_WO_Time1.Text.Trim() + "' and '" + TextBox_WO_Time2.Text.Trim() + "'";
        ...
        string JJBL_ShTime = (TextBox_tibaoshijian1.Text.Trim() == "" && TextBox_tibaoshijian2.Text.Trim() == "") ? " and 1=1 " : " and JSBL_ShTime between   '" + TextBox_tibaoshijian1.Text.Trim() + "' and '" + TextBox_tibaoshijian2.Text.Trim() + "'";
        condition = JJBL_Man + JJBL_NO + JJBL_ShMan + JJBL_ShTime + JJBL_State + JJBL_Time;
        GridView_WOmain.DataSource = ws.S_JSBL(condition);
    ...
    protected void Btn1_Click(object sender, EventArgs e)
    {
        try
        {
            ws.I_JSBL(Session["UserName"].ToString().Trim(), TextBox1.Text.Trim());
    ...
    protected void CheckBoxAll2_CheckedChanged(object sender, EventArgs e)
    {
        for (int i = 0; i <= GridView_WOmain.Rows.Count - 1; i++)
        {
            ...
            if (GridView_WOmain.DataKeys[i].Values["JSBL_State"].ToString().Trim() == "财务待审")
            {
    ...
    protected void Button_AddPTToSeries_Click(object sender, EventArgs e)//批量审核
    {
        ...
        for (int i = 0; i <= GridView_WOmain.Rows.Count - 1; i++)
        {
            ...
            if (CheckBox.Checked == true)
            {
                ...
                {
                    Label14.Text = GridView_WOmain.DataKeys[i].Values["JSBL_NO"].ToString().Trim();
                    this.Label_PLID.Text = GridView_WOmain.DataKeys[i].Values["JSBL_ID"].ToString().Trim();
                }
                else
                {
                    string[] a;
                    if (Label14.Text.Contains(","))
                    {
                        ...
                        int id = Array.IndexOf(a, GridView_WOmain.DataKeys[i].Values["JSBL_NO"].ToString().Trim()); // 这里的1就是你要查找的值
                        if (id == -1)
                        {
                            Label14.Text = Label14.Text + "," + GridView_WOmain.DataKeys[i].Values["JSBL_NO"].ToString().Trim();
                            Label_PLID.Text = Label_PLID.Text + "," + GridView_WOmain.DataKeys[i].Values["JSBL_ID"].ToString().Trim();
                        }
                    }
                    else
                    {
                        if (Label14.Text != GridView_WOmain.DataKeys[i].Values["JSBL_NO"].ToString().Trim())
                        {
                            Label14.Text = Label14.Text + "," + GridView_WOmain.DataKeys[i].Values["JJSBL_NO"].ToString().Trim();
                            Label_PLID.Text = Label_PLID.Text + "," + GridView_WOmain.DataKeys[i].Values["JSBL_ID"].ToString().Trim();
                        }
                    }
    ...
    protected void GridView_WOmain_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "dele")//
        {
            ...
            try
            {
                ws.D_JSBL(new Guid(GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_ID"].ToString()));
        ...
        if (e.CommandName == "Detail12")//
        {
            ...
            DataSet ds_jjlx = wosl.S_TimeSalaryDateDetail_SalaryTimeItem("00000000-0000-0000-0000-000000000000", "");
            DataTable dt = ds_jjlx.Tables[0];
            DataRow dr = dt.NewRow();
            dr["STI_ID"] = "00000000-0000-0000-0000-000000000000";
            dr["STI_Name"] = "不计计时工资";
            dt.Rows.InsertAt(dr, 0);
            DropDownList12.DataSource = dt;
            DropDownList12.DataTextField = "STI_Name";
            DropDownList12.DataValueField = "STI_ID";
            DropDownList12.DataBind();
            if (this.label_pagestate.Text.Contains("sh"))
            {
                if (GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_State"].ToString().Trim() != "财务待审")
                {
                    TextBox7.Enabled = false;
                    pbtg.Visible = false;
                    pbbh.Visible = false;
            ...
            {
                if (GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_State"].ToString().Trim() != "财务驳回" && GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_State"].ToString().Trim() != "已建立")
                {
            ...
            Label_JJBL_ID.Text = GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_ID"].ToString();
            TextBox8.Text = "";
            TextBox2.Text = "";
            TextBox3.Text = "";
            TextBox9.Text = "";
            TextBox10.Text = "";
            TextBox15.Text = "";
            label_Date.Text = GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_NO"].ToString();
            Label41.Text = GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_ShMan"].ToString();
            Label42.Text = (GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_ShTime"].ToString() == "&nbsp;" || GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_ShTime"].ToString().Trim() == "") ? "" : Convert.ToDateTime(GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_ShTime"].ToString().Trim()).ToString("yy-MM-dd HH:mm");//人事审核时间;
            label52.Text = GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_ShRes"].ToString();
            TextBox7.Text = GridView_WOmain.DataKeys[row.RowIndex].Values["JSBL_ShSugg"].ToString();
    ...
    protected void GridView_WOmain_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (drv["JSBL_State"].ToString().Trim() == "财务待审")
            {
            ...
            if (drv["JSBL_State"].ToString().Trim() == "财务驳回" || drv["JSBL_State"].ToString().Trim() == "已建立")
            {
    ...
    public void databind_detail()
    {
        ...
        string SPI_Name = TextBox15.Text.Trim() == "" ? " and 1=1 " : " and isnull(STI_Name,'不计计时工资') like '%" + TextBox15.Text.Trim() + "%' ";

        condition = WO_Num + HRDD_StaffNO + HRDD_Name + WO_ProType + PBC_Name + " and JSBL_ID='" + this.Label_JJBL_ID.Text.Trim() + "'" + SPI_Name;
        GridView1.DataSource = ws.S_JSBLD(condition);
        GridView1.DataBind();
        UpdatePanel2.Update();
    ...
    protected void pbtg_Click(object sender, EventArgs e)
    {
        if (Label_type.Text.Trim() == "0")
        {
            try
            {
                ...
                ws.U_JSBL_SH(new Guid(this.Label_JJBL_ID.Text.Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "通过");
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('审核成功！')", true);

                string message = "ERP系统信息：" + Session["Department"].ToString() + "的" + Session["UserName"].ToString() + "于" + DateTime.Now + "审核通过了计时补录单： " + label_Date.Text + " ！";

                string ms = "";
                try
                {
                    ms = RTXhelper.Send(message, "计时补录提报");
                }
                catch (Exception)
                {
        ...
        else
        {
            ...
            try
            {
                ...
                {
                    ws.U_JSBL_SH(new Guid(a[i].Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "通过");

                }
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('审核成功！')", true);
                string message = "ERP系统信息：" + Session["Department"].ToString() + "的" + Session["UserName"].ToString() + "于" + DateTime.Now + "审核通过了计时补录单： " + Label14.Text + " ！";
    ...
    protected void pbbh_Click(object sender, EventArgs e)
    {
        if (Label_type.Text.Trim() == "0")
        {
            try
            {
                ...
                ws.U_JSBL_SH(new Guid(this.Label_JJBL_ID.Text.Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "不通过");
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('审核成功！')", true);
                string message = "ERP系统信息：" + Session["Department"].ToString() + "的" + Session["UserName"].ToString() + "于" + DateTime.Now + "审核驳回了计时补录单： " + label_Date.Text + " ！";
        ...
        else
        {
            ...
            try
            {
                for (int i = 0; i < a.Length; i++)
                {
                    ws.U_JSBL_SH(new Guid(a[i].Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "不通过");

                }
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('审核成功！')", true);
                string message = "ERP系统信息：" + Session["Department"].ToString() + "的" + Session["UserName"].ToString() + "于" + DateTime.Now + "审核驳回了计时补录单： " + Label14.Text + " ！";
    ...
    protected void Btn8_Click(object sender, EventArgs e)
    {
        try
        {
            if (this.Label_HRDD_ID.Text.Trim().Contains(","))
            {
                ...
                for (int i = 0; i < a.Length; i++)
                {
                    ws.I_JSBLD(new Guid(this.Label_WOD_ID.Text.Trim()), new Guid(a[i].Substring(1).Substring(0, a[i].Length - 2)), new Guid(this.Label_JJBL_ID.Text.Trim()));
            ...
            else
            {
                ...
                {
                    ws.I_JSBLD(new Guid(this.Label_WOD_ID.Text.Trim()), new Guid(Label_HRDD_ID.Text.Substring(1).Substring(0, Label_HRDD_ID.Text.Length - 2)), new Guid(this.Label_JJBL_ID.Text.Trim()));
    ...
    protected void Btn3_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i <= GridView1.Rows.Count - 1; i++)
        {
            ...
            if (CheckBox.Checked == true)
            {
                try
                {
                    ws.D_JSBLDetail(new Guid(GridView1.DataKeys[i].Values["补录ID"].ToString()));
    ...
    protected void DropDownList12_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i <= GridView1.Rows.Count - 1; i++)
        {
            ...
            if (CheckBox.Checked == true)
            {
                try
                {
                    ws.U_JSBLDetail_SPI(new Guid(GridView1.DataKeys[i].Values["补录ID"].ToString()), new Guid(DropDownList12.SelectedValue.ToString().Trim()));
    ...
    protected void Btn15_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i <= this.GridView1.Rows.Count - 1; i++)
        {
            ...
            try
            {
                ...
                t1 = ((TextBox)(GridView1.Rows[i].FindControl("txtPlan1"))).Text.Trim() == "" ? 0 : Convert.ToDecimal(((TextBox)(GridView1.Rows[i].FindControl("txtPlan1"))).Text.Trim());
                ws.U_JSBLD_Num(t, id, t1);
            }
            catch (Exception)
            {
    ...
    protected void Btn16_Click(object sender, EventArgs e)
    {
        try
        {
            ws.U_JSBL_TJ(new Guid(Label_JJBL_ID.Text));
            ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('提交成功！')", true);
            string message = "ERP系统信息：" + Session["Department"].ToString() + "的" + Session["UserName"].ToString() + "于" + DateTime.Now + "提交了计时补录单： " + this.label_Date.Text + " ,请审核！";

            string ms = "";
            try
            {
                ms = RTXhelper.Send(message, "计时补录审核");
            }
            catch (Exception)
            {
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('" + ms + "')", true);
