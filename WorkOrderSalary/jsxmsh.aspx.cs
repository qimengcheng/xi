...
public partial class WorkOrderSalary_jsxmsh : System.Web.UI.Page
{
    WSD ws = new WSD();
    ...
    public void databind_zhubiao()
    {
        ...
        string PA_RMan = TextBox_chushengren.Text.Trim() == "" ? " and 1=1 " : " and PTA_RMan like '%" + TextBox_chushengren.Text.Trim() + "%' ";
        string PA_State = DropDownList1.SelectedItem.Text.Trim() == "请选择" ? " and 1=1 " : " and isnull(PTA_State,'财务待审') like '%" + DropDownList1.SelectedItem.Text.Trim() + "%' ";
        ...
        string PA_RTime = (TextBox_tibaoshijian1.Text.Trim() == "" && TextBox_tibaoshijian2.Text.Trim() == "") ? " and 1=1 " : " and PTA_RTime between   '" + TextBox_tibaoshijian1.Text.Trim() + "' and '" + TextBox_tibaoshijian2.Text.Trim() + "'";


        condition = STI_Name + PBC_Name + PA_RMan + PA_State + PA_Date + PA_RTime;
        GridView_WOmain.DataSource = ws.S_JSXMSH(condition);
    ...
    public void databind_detail()
    {
        ...
        GridView1.DataSource = ws.S_JSXMSH_Detail(Label_SPI_ID.Text.Trim(), label_Date.Text.Trim(), condition);
    ...
    protected void pbtg_Click(object sender, EventArgs e)
    {
        if (Label_type.Text.Trim() == "0")
        {
            try
            {
                ...
                {
                    ws.I_RelatedTimeAuditor(new Guid(Label_SPI_ID.Text.Trim()), Convert.ToDateTime(label_Date.Text.Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "通过");
                    ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('审核成功！')", true);
                }
                else
                {

                    ws.U_RelatedTimeAuditor(new Guid(Label21.Text.Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "通过");
                    ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('审核成功！')", true);
                }
            }
            catch (Exception)
        ...
        else
        {
            ...
            try
            {
                ...
                {
                    ws.I_RelatedTimeAuditor_Piliang(new Guid(a[i].Remove(0, 10).Trim()), Convert.ToDateTime(a[i].Substring(0, 10).Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "通过");
    ...
    protected void pbbh_Click(object sender, EventArgs e)
    {
        if (Label_type.Text.Trim() == "0")
        {
            try
            {
                ...
                if (Label21.Text.Trim() == "")
                {
                    ws.I_RelatedTimeAuditor(new Guid(Label_SPI_ID.Text.Trim()), Convert.ToDateTime(label_Date.Text.Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "不通过");
                    ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('审核成功！')", true);
                }
                else
                {

                    ws.U_RelatedTimeAuditor(new Guid(Label21.Text.Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "不通过");
                    ScriptManager.RegisterStartupScript(Page, typeof(Page), "alert", "alert('审核成功！')", true);
                }
            }
            catch (Exception)
        ...
        else
        {
            ...
            try
            {
                for (int i = 0; i < a.Length; i++)
                {
                    ws.I_RelatedTimeAuditor_Piliang(new Guid(a[i].Remove(0, 10).Trim()), Convert.ToDateTime(a[i].Substring(0, 10).Trim()), Session["UserName"].ToString(), TextBox7.Text.Trim(), "不通过");
