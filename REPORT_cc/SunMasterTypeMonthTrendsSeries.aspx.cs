...
public partial class Laputa_SunMasterTypeMonthTrendsSeries : System.Web.UI.Page
{
    ...
    protected void SearchInStoreMain(object sender, EventArgs e)
    {
        ...
        GridView2.DataSource = lpp.Query_SunMasterMonthSeries(DropDownList1.SelectedValue);
        GridView2.DataBind();
        Panel3.Visible = true;
    ...
    protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Gra")
        {
            ...
            DataSet ds = lpp.Query_SunMasterMonthSeries(DropDownList1.SelectedValue);
            DataTable dt = new DataTable();
            if (ds.Tables[0].Select("产品系列='" + (e.CommandArgument) + "'").Any())
            {
                dt = ds.Tables[0].Select("产品系列='" + (e.CommandArgument) + "'").CopyToDataTable();
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    Chart1.Series[0].Points.AddXY(dr["年份"], dr["出站数"]);
                    Chart1.Series[1].Points.AddXY(dr["年份"], dr["合格率"]);
                }
            }
            ...
            //绑定数据
            Chart1.DataBind();
            Chart1.Titles[0].Text = e.CommandArgument + "-" + DropDownList1.SelectedValue + "月历年塑封桥、浇灌桥和模块事业部产品产量和合格率趋势图";
    ...
    protected void print(object sender, EventArgs e)
    {
        ...
        Response.Redirect("~/report_cc/SunMasterTypeMonthTrendsSeriesPrint.aspx?month="+DropDownList1.SelectedValue);
    }
    ...
    protected void Chart1_Load(object sender, EventArgs e)
    {
        ...
        //; //设置图表Y轴对应项
        //DataTable dt=new DataTable();
        //if (ds.Tables[0].Select("类型='塑封桥' and 月份=" + (DropDownList1.SelectedValue)).Any())
        //{
        //    dt = ds.Tables[0].Select("类型='塑封桥' and 月份=" + (DropDownList1.SelectedValue)).CopyToDataTable();
        //    foreach (DataRow dr in dt.AsEnumerable())
        //    {
        //        Chart1.Series[0].Points.AddXY(dr["年份"] ,dr["出站数"]);
        ...
        //Chart1.Titles[0].Text = DropDownList1.SelectedValue + "月历年塑封桥、浇灌桥和模块事业部产品产量和合格率趋势图";
