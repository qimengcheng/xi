...
public partial class Laputa_ProjectInfoMaterialPrint: Page
{
    ...
    {
        Response.Redirect("~/report_cc/ProjectInfoMaterial.aspx");
    }
    protected void GridView2_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                SqlDataReader reader = lpp.Query_ProjectReportMaterialBad(e.Row.Cells[3].Text, e.Row.Cells[1].Text,
                ...
                while (reader.Read())
                {
                    var dc = new DataColumn {ColumnName = reader["WOBP_Type"].ToString()};
                    dt.Columns.Add(dc);
